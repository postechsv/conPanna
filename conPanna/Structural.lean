...
elab_rules : command
  | `(structural $name:ident where $laws:structuralLaw*) => do
      ...
      for operation in commutativeOperations do
        elabCommand (← `(command|
          instance : Structural.HasComm $name $operation := ⟨True.intro⟩))
      for operation in associativeOperations do
        elabCommand (← `(command|
          instance : Structural.HasAssoc $name $operation := ⟨True.intro⟩))
      for (operation, element) in identities do
        elabCommand (← `(command|
          instance : Structural.HasIdentity $name $operation $element :=
            ⟨True.intro⟩))
